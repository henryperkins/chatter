...
class ModelForm(FlaskForm):
    ...
    def validate_temperature(self, field):
        if self.requires_o1_handling.data:
            if field.data is not None:
                raise ValidationError("Temperature must not be set for models requiring o1-preview handling.")
        else:
    ...
    def validate_max_tokens(self, field):
        if self.requires_o1_handling.data:
            if field.data is not None:
                raise ValidationError("Max tokens must not be set for models requiring o1-preview handling.")
        else:
    ...
    def validate_max_completion_tokens(self, field):
        ...
        if self.requires_o1_handling.data and field.data is None:
            raise ValidationError("Max completion tokens must be specified for o1-preview models.")
