### Overview:
Azure OpenAI's o-series models (`o3-mini`, `o1`, `o1-mini`, `o1-preview`) are specialized reasoning models with distinct requirements and capabilities. This guide outlines their key requirements and provides guidance for effective implementation.

---

### **Key Requirements of o-series Models:**

1. **System Messages and Developer Messages:**  
   - `o3-mini` and `o1`: Support both system messages and developer messages
   - `o1-mini` and `o1-preview`: Do not support system messages
   - Developer messages are functionally equivalent to system messages for all o-series models

2. **Streaming Support:**  
   - `o3-mini`: Supports streaming responses
   - `o1`, `o1-mini`, `o1-preview`: Do not support streaming
   - For non-streaming models, use `client.chat.completions.create` to receive complete responses

3. **Temperature Fixed at 1:**  
   - All o-series models require temperature=1.0
   - This parameter cannot be modified

4. **Token Parameters:**  
   - Use `max_completion_tokens` instead of `max_tokens`
   - Token limits vary by model:
     - `o3-mini`: 150,000 max tokens, 75,000 completion tokens
     - `o1`: 200,000 max tokens, 100,000 completion tokens
     - `o1-mini`: 100,000 max tokens, 50,000 completion tokens
     - `o1-preview`: 128,000 max tokens, 32,768 completion tokens

5. **API Version Requirements:**  
   - Use `2024-12-01-preview` or newer
   - `o3-mini` and `o1` support `2025-01-01-preview`
...
# Example: o3-mini with streaming
response = client.chat.completions.create(
    model="your-o3-mini-deployment",
    messages=[
        {"role": "developer", "content": "Formatting re-enabled - code output should be wrapped in markdown."},
...
# Example: o1 without streaming
response = client.chat.completions.create(
    model="your-o1-deployment",
    messages=[
        {"role": "developer", "content": "Formatting re-enabled - please enclose code blocks with appropriate markdown tags."},
        {"role": "user", "content": "Your prompt here"}
    ],
    temperature=1,
...
1. **Reasoning Effort Parameter:**
   - Available for `o3-mini` and `o1` models
   - Options: "low", "medium" (default), "high"
   - Higher effort means more thorough reasoning but increased token usage and latency
   - Affects the number of reasoning tokens in completion_tokens_details

2. **Markdown Formatting:**
   - By default, o-series models may not use markdown formatting
   - To enable markdown (especially for code blocks):
     - Add "Formatting re-enabled" at the start of developer messages
     - For better results, use more specific instructions like:
       - "Formatting re-enabled - please enclose code blocks with appropriate markdown tags"
       - "Formatting re-enabled - code output should be wrapped in markdown"
   - `o1` models may need more explicit formatting instructions than `o3-mini`

3. **Feature Support Matrix:**
   - Function/Tool Calling: Supported by `o3-mini` and `o1`
   - Vision Support: Only available in `o1`
   - Structured Outputs: Supported by `o3-mini` and `o1`
   - Developer Messages: Supported by all models
   - System Messages: Only supported by `o3-mini` and `o1`

4. **Error Handling:**
   - Implement robust error handling for:
     - Invalid parameters (temperature ≠ 1)
     - Unsupported features per model
...
### **Testing Checklist:**
- Verify correct API version for each model
- Confirm temperature is fixed at 1.0
- Test with appropriate token limits per model
- Validate streaming support (o3-mini only)
- Check reasoning_effort parameter behavior
- Test markdown formatting with developer messages
- Verify system message support based on model
- Monitor reasoning_tokens in completion_tokens_details

---

### **References:**
- [Azure OpenAI o-series Models Documentation](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/reasoning?tabs=python#usage)
- [Azure OpenAI API Reference](https://learn.microsoft.com/en-us/azure/ai-services/openai/reference)
- [o-series Model Capabilities](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models#o-series-models)
