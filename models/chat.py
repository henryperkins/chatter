...
class Chat:
    ...
    def can_access_chat(chat_id: str, user_id: int, user_role: str) -> bool:
        ...
        if user_role == "admin":
            try:
                with db_session() as db:
                    query = text("SELECT COUNT(1) FROM chats WHERE id = :chat_id AND (is_deleted = FALSE OR is_deleted IS NULL)")
    ...
    def is_title_default(chat_id: str) -> bool:
        ...
        with db_session() as db:
            try:
                query = text("SELECT title FROM chats WHERE id = :chat_id AND (is_deleted = FALSE OR is_deleted IS NULL)")
    ...
    @staticmethod
    def update_model_id(chat_id: str, model_id: int) -> None:
        """Update the model ID for a chat."""
        with db_session() as db:
            try:
                query = text("""
                    UPDATE chats
                ...
                db.commit()
                logger.info(f"Updated model ID to {model_id} for chat {chat_id}")
            except Exception as e:
                db.rollback()
                logger.error(f"Failed to update model ID for chat {chat_id}: {e}")
                raise
            ...
            try:
                query = text(
                    ...
                    SELECT
                        ...
                        TO_CHAR(c.created_at, 'YYYY-MM-DD HH24:MI:SS') as timestamp,
    ...
    def get_by_id(chat_id: str) -> Optional["Chat"]:
        ...
        with db_session() as db:
            try:
                query = text("SELECT id, user_id, title, model_id FROM chats WHERE id = :chat_id AND (is_deleted = FALSE OR is_deleted IS NULL)")
                row = db.execute(query, {"chat_id": chat_id}).mappings().first()
    ...
    def get_model(chat_id: str) -> Optional["Model"]:
        ...
        with db_session() as db:
            try:
                query = text("SELECT model_id FROM chats WHERE id = :chat_id AND (is_deleted = FALSE OR is_deleted IS NULL)")
                row = db.execute(query, {"chat_id": chat_id}).mappings().first()
    ...
    @staticmethod
    def add_message(chat_id: str, role: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> int:
        """
